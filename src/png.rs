...
impl Png {
    ...
    pub fn append_chunk(&mut self, chunk: Chunk) {
         let search = self.chunks
            .iter()
            .find(|c| c.chunk_type().bytes() == chunk.chunk_type().bytes());
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        match index {
            ...
            None => Err(anyhow!("No chunk exists"))
        }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        loop {
            ...
            if chunk.chunk_type().bytes() == "IEND".as_bytes() {
                chunks.push(chunk);
                break;
            } else {
                data = &data[usize::try_from(chunk.length())? + 12..];
                chunks.push(chunk);
            }
        }
...
mod tests {
    ...
    fn testing_chunks() -> Vec<Chunk> {
        ...
        chunks.push(chunk_from_strings("IEND","Ending chunk").unwrap());
